...
def main():
    ...
    # results[distance_metric][topology][k][model]
    results = {dm: {} for dm in DISTANCE_METRICS}
    overall_t0 = time.time()
    for distance_metric in DISTANCE_METRICS:
        for topology in TOPOLOGIES:
            # the temporal topology has no spatial component, so haversine and
            # euclidean produce identical graphs -- skip the redundant re-run.
            if distance_metric == "haversine" and topology == "temporal":
                continue
            results[distance_metric][topology] = {}
            for k in NEIGHBORS_LIST:
                ...
                print(f"\n--- metric={distance_metric}, topology={topology}, k={k}, edges={data.num_edges}, edge_attr_dim={edge_dim} ---")
                results[distance_metric][topology][k] = run_models(data, configs, weights, device)
        print(f"\n  -- metric={distance_metric} done in {(time.time() - overall_t0):.1f}s elapsed --")
    ...
    for distance_metric in DISTANCE_METRICS:
        ...
        print()
        for topology in TOPOLOGIES:
            if topology not in results[distance_metric]:
                continue
            for model_name in ["GCN", "GAT", "TransformerConv", "RGCN"]:
                row_data = []
                present = False
                for k in NEIGHBORS_LIST:
                    if model_name in results[distance_metric][topology][k]:
                        s = results[distance_metric][topology][k][model_name]
    ...
    with open(OUTPUT_CSV, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["distance_metric", "topology", "k", "model", "f1_mean", "f1_std", "acc_mean", "acc_std", "params", "time_seconds"])
        for distance_metric in DISTANCE_METRICS:
            for topology in TOPOLOGIES:
                if topology not in results[distance_metric]:
                    continue
                for k in NEIGHBORS_LIST:
                    for model_name, s in results[distance_metric][topology][k].items():
                        writer.writerow([distance_metric, topology, k, model_name, f"{s['f1_mean']:.6f}", f"{s['f1_std']:.6f}",
